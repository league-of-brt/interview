...
于是在ASCII的基础上，1980年的GB2312就支持了简体中文。有简体，没有繁体怎么行？那就在拓展字符集咯。1984年的BIG5就支持了繁体字。

那么问题来了，拓展字符的需求是无限的，人们刚刚确定了一个字符集，就发现不支持某些字符，又要去确定下一个字符集.....这样就周而复始没个完了，非常不方便开发使用。

所以，与其不停退出字符集，不如制定全球通用的标准字符集，于是1994年公布全球第一个通用字符集Unicode。
...
string变量，结构中有一个data指针，指向一块内存：

|01|02|03|04|05|06|07|08|09|
|----|----|----|----|----|----|----|----|----|
|e|g|g|o|..|..|..|..|..|

那么问题又来了，我这个data指针只能指向开头的这个`e`，我怎么知道这个字符串到内存的哪里结束呢？
...
我通过data指针拿str的数值的时候，一直连续的读过去，不就读成`eggoab`了吗。
...
golang选了更加聪明的方式，string结构体中除了data外，还存有一个变量len。data指针指向内存中字符串的开头，len决定了读取的字节数。
...
2. 字符串变量可以共用底层字符串内容，比如：`str1 := "eggo", str2 := "go"`，str2中的data指针可能指向内存中str1的`go`中的`g`，如果我们能修改s1的内容，那么s2的内容也会被修改，这显然是不能预测的。
